...
Beyond the category, four cheap deterministic axes plus a
...
### Generated-content mark — `generated-scan`

Carried patches on old-style `1.0` diffs often regenerate autotools
output — `configure`, `Makefile.in`, `ltmain.sh` — alongside a real
hand edit, so a small reviewable residue sits buried under enormous
boilerplate. A pure scan marks the files that **claim** to be
generator output, via two independent signals: an exact basename
match (24 names in v1 — `configure`, `Makefile.in`, `aclocal.m4`,
`ltmain.sh`, `config.guess`, …) at any path depth, or a generator
banner (`Generated by GNU Autoconf 2.59`, `generated by automake
1.9.6`, `@generated`, …) visible on any line of the file's touched
region. Editor/patch-tool backup suffixes (`.ori`, `.orig`, `.bak`,
`.rej`, a trailing `~`) are stripped before matching, so a backup is
classified as whatever it is a copy of in either direction —
`Makefile.in.ori` marks, `Makefile.am.ori` does not.

`Makefile.in` carries its own corroboration tier: its name signal
counts only alongside a banner hit in the same region. Measured,
~880 of the corpus's 1,280 `Makefile.in` matches are hand-written
`AC_OUTPUT` templates rather than automake output — a genuine
regeneration rewrites the file from line 1 and so shows its banner
anyway, so the requirement costs the target population nothing while
...
is replacing. Line-order-only selection had reported the pre-patch,
about-to-be-replaced version in 59 of 61 multi-banner regions
measured.
...
banner-only marks bringing the marked total to **442 fingerprints**
...
a marked file stays in the record, badged, never dropped. The claim
is that a file *says* it is generator output, not that regeneration
was verified — upgrading "claims" to "verified" is deliberately out
of scope. The known exception is the libtool source package itself:
its own `libtool.m4` / `ltmain.sh` / `libtool.texi` are hand-written
upstream source, not copied generator output, so the name claim is
wrong there by construction (n=3, no rule proposed — a reader of the
mark needs to know this). A broader generic `DO NOT EDIT` /
"automatically generated" family was measured (1,011 hits, no
generator or version, dominated by an unrelated Perl timezone-module
population) and rejected for v1 on volume-without-discrimination
grounds. (`generated.py`)
