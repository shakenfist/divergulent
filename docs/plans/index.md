...
| [PLAN-published-cache.md](PLAN-published-cache.md) | 1. Central builder + measure ◐ (~0.73 MB; ~95 min cold, ~80 s incremental; spot-check left) · 2. Bundle-backed sources + fallback ✓ (`--bundle`) · 3. `cache pull` ⋯ · 4. Signing + verify ⋯ · 5. Publish to GitHub Releases ⋯ | Phase 2 implemented | Compute staleness + divergence for the whole Debian archive once, centrally, and publish a small signed bundle that clients download whole (private) and match locally — turning a cold run into seconds. Builder first to confirm size/timing before delivery. |
