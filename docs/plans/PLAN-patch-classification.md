...
| 6. BTS / upstream cross-reference | [PLAN-patch-classification-phase-06-cross-reference.md](PLAN-patch-classification-phase-06-cross-reference.md) | **Planned** — the `external` rule tier: verify author-declared CVE/bug claims against Debian's own records (Security Tracker first, BTS second), bulk-pinned snapshots with recorded freshness (the reserved `input_snapshot`/`input_fresh_until` columns), settling `security` on strong corroboration and only *flagging* contradictions. Not implemented |
