...
| 7. Pre-push audit + closeout | [PLAN-patch-classification-phase-07-closeout.md](PLAN-patch-classification-phase-07-closeout.md) | In progress — runs `PUSH-AUDIT.md` over the accumulated diff of phases 1–6, moves the two outstanding measurements (phase 6 E6's verdict split, phase 4 R4's risk-gate validation) to the issue tracker, writes the operator runbook, and marks this plan `Complete` |
