...
| 5. Classification bundle & client display | [PLAN-patch-classification-phase-05-bundle.md](PLAN-patch-classification-phase-05-bundle.md) | **Plan drafted** — publish a signed fingerprint→verdict bundle (gzipped JSON, like the divergence cache) from a *committed JSONL export* of the ledger (never the sqlite); client hashes patch bodies and looks them up, running no classifier |
