...
Everything from here is the **patch classification pipeline**, which is
documented in [docs/patch-classification.md](docs/patch-classification.md):
`divergulent/classify/` crawls the archive's patched packages into a
content-addressed corpus, classifies each fingerprint deterministically
(keeping the author's DEP-3 *claim* strictly separate from the diff's
*content*, so their disagreement is the signal), records verdicts in an
append-only decision ledger whose current verdict is always derived
rather than stored, fills the reserved LLM and human seats through
claim-blind triage with independent adversarial verification, and
publishes the result as a committed JSONL export. `divergulent-classify`
is the single front for all of it.

Two things about that pipeline are load-bearing conventions rather than
implementation detail, so they are repeated here:

- **The classification ledger is the source of truth, not the cache.**
  The divergence cache is a pure function of the archive and CI
  regenerates it; the ledger embeds irreproducible human and
  verified-LLM verdicts. It reaches CI as a committed JSONL export,
  never as sqlite — binary diffs are unreviewable, unmergeable, and
  bloat git.
- **A decision is superseded, never edited or deleted**, and an
  observation (a dangerous-construct flag) never becomes a category.
...
human (priority band below risk, above provenance), and the review UI
badges it. Both tiers ask `injection_suspect_fingerprints`, so a new LLM
