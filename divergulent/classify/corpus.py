...
# A fetch returns ``(source_format, texts, changelog_date, binaries, description)``
# like ``fetch_source_details``: texts is ``{name: raw_text}`` (patched), ``{}`` (clean
...
# description is the one-line ``debian/control`` synopsis (review orientation) or None.
FetchResult = tuple["str | None", "dict[str, str] | None", "str | None", "list[str]", "str | None"]
...
def _process(source_package: str, version: str, *, corpus_dir: str,
    ...
    """
    try:
        source_format, texts, changelog_date, binaries, description = fetch(source_package, version)
        ...
        package_row = {
            ...
            'description': None, 'error': 'fetch-error: %s' % exc}
    ...
    package_row = {
        ...
        'changelog_date': changelog_date, 'binaries': binaries,
