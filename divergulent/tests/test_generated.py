...
The phase-3 routing helpers sit alongside them: ``project_residue_first``'s residue-first
shape (verbatim segments in diff order ahead of every note, correct preamble arithmetic, no
hunk content from a marked file) and its identity behaviour on a body with nothing to
project, with a joint assertion tying its segmentation to ``content._parse_sections``' so
the two can never disagree; and ``residue_unlocked_fingerprints`` composing reviewability's
``oversized`` set with the mark's residue, including the threshold edge and every way a
fingerprint stays locked.

The phase-4 DISPLAY helper follows them: ``construct_tally`` splitting one body's
dangerous-construct hits into marked files versus the residue -- the marked hit, the residue
hit with its ``(path, detail)``, the type gate honoured in both directions (a construct in a
doc file, and one in build-typed ``configure``, count nowhere, exactly as neither flagged
when the observations were recorded), an empty marked set putting everything in the residue,
and a body with no constructs tallying zero -- plus the single-sourcing bar: a pattern
monkeypatched into ``rules._DANGEROUS_PATTERNS`` (and the shell-only table) flows straight
into the tally, because it reaches the LIVE table objects through ``rules`` and defines none
of its own.
...
# ---------------------------------------------------------------------------
# Residue-first projection: what the LLM passes are shown, and what they are told
# ---------------------------------------------------------------------------

def _evidence_files(text):
    """The mark's per-file evidence for ``text`` -- exactly what ``generated_marks`` hands on.

    Round-tripped through ``evidence_for``'s JSON rather than read off the scan objects, so
    the projection is exercised against the dict shape the ledger really stores (signals as
    a list, ``generator``/``version`` as explicit nulls).
    """
    return json.loads(generated.evidence_for(generated.scan(text)))['files']


# The residue of GATOS_SHAPED, verbatim: a backup of a hand-written input and the one real
# source change, in diff order.  Sliced from the fixture's own builder so the expectation
# cannot drift from the input.
RESIDUE_SEGMENTS = (
    _file_diff('Makefile.am.ori', ['+EXTRA_DIST = notes']),
    _file_diff('src/foo.c', ['+    int fixed = 1;']),
)
...
def _decorated(path, diff):
    """``diff`` behind the git decoration lines a git-format patch puts before its headers."""
    return 'diff --git a/%s b/%s\nindex 1111111..2222222 100644\n%s' % (path, path, diff)


# The same shape as GATOS_SHAPED but git-decorated: the decoration belongs to the segment
# whose headers FOLLOW it, so a residue file must keep its own and never inherit a marked
# file's.
DECORATED = ''.join([
    _decorated('configure', _file_diff('configure', [
        ' # Generated by GNU Autoconf 2.59.', '-ac_old', '+ac_new'])),
    _decorated('src/foo.c', _file_diff('src/foo.c', ['+    int fixed = 1;'])),
])


class ProjectResidueFirstTestCase(testtools.TestCase):

    def setUp(self):
        super().setUp()
        self.files = _evidence_files(GATOS_SHAPED)
        self.result = generated.project_residue_first(GATOS_SHAPED, self.files)

    def _notes(self):
        return [line for line in self.result.text.splitlines() if line.startswith('[generated:')]

    def test_the_result_reports_the_projection(self):
        self.assertTrue(self.result.projected)
        self.assertEqual(2, self.result.omitted_files)
        self.assertEqual(8, self.result.omitted_changed)

    def test_the_preamble_is_the_first_line_and_carries_both_counts(self):
        # X of Y: the generated changed lines out of the whole diff's changed lines.
        self.assertEqual(
            '2 generated-claiming files not shown (8 of 10 changed lines); '
            'hand-written residue follows.',
            self.result.text.splitlines()[0])

    def test_the_preamble_counts_are_the_scan_arithmetic(self):
        scan = generated.scan(GATOS_SHAPED)
        self.assertEqual(scan.generated_changed, self.result.omitted_changed)
        self.assertIn('(%d of %d changed lines)' % (scan.generated_changed, scan.total_changed),
                      self.result.text)

    def test_residue_segments_are_verbatim_and_in_diff_order(self):
        first = self.result.text.find(RESIDUE_SEGMENTS[0])
        second = self.result.text.find(RESIDUE_SEGMENTS[1])
        self.assertNotEqual(-1, first, 'the first residue segment is not present verbatim')
        self.assertNotEqual(-1, second, 'the second residue segment is not present verbatim')
        self.assertLess(first, second)

    def test_the_residue_comes_before_every_note(self):
        self.assertLess(self.result.text.find(RESIDUE_SEGMENTS[1]),
                        self.result.text.find('[generated:'))

    def test_no_hunk_content_from_a_marked_file_survives(self):
        # The point of the projection: the cap's budget stops being spent on generator
        # output.  Header lines go too -- a marked file appears ONLY as its note.
        for absent in ('ac_new_one', 'ac_old_one', 'CFLAGS = -O2', 'CFLAGS = -O1',
                       'Generated by GNU Autoconf', 'generated by automake',
                       '--- a/configure', '+++ b/src/Makefile.in'):
            self.assertNotIn(absent, self.result.text)

    def test_one_note_per_marked_file_in_diff_order(self):
        self.assertEqual(
            ['[generated: configure — 6 changed lines, signals banner+name, autoconf 2.59]',
             '[generated: src/Makefile.in — 2 changed lines, signals banner+name, '
             'automake 1.9.6]'],
            self._notes())

    def test_a_note_without_a_generator_omits_the_clause(self):
        # A name-only match carries no version evidence at all; the note says what it knows
        # and stops, rather than printing a pair of nulls.
        body = _file_diff('configure', ['+ac_new_one']) + _edit('src/foo.c')
        result = generated.project_residue_first(body, _evidence_files(body))
        self.assertEqual(['[generated: configure — 1 changed lines, signals name]'],
                         [line for line in result.text.splitlines()
                          if line.startswith('[generated:')])

    def test_decoration_lines_ride_with_the_segment_they_precede(self):
        result = generated.project_residue_first(DECORATED, _evidence_files(DECORATED))
        self.assertTrue(result.projected)
        # The residue file keeps its own decoration, byte for byte ...
        self.assertIn(_decorated('src/foo.c', _file_diff('src/foo.c', ['+    int fixed = 1;'])),
                      result.text)
        # ... and does not inherit the marked file's.
        self.assertNotIn('diff --git a/configure b/configure', result.text)

    def test_segments_reconstruct_the_body(self):
        # Nothing is dropped or duplicated between file boundaries: the segments ARE the
        # diff, which is what lets the residue be quoted verbatim.
        self.assertEqual(GATOS_SHAPED, ''.join(generated._file_segments(GATOS_SHAPED)))
        self.assertEqual(DECORATED, ''.join(generated._file_segments(DECORATED)))

    def test_segmentation_agrees_with_the_scanners_sections(self):
        # The joint assertion: the files the projection shows are exactly the files the
        # scanner did not mark, named by the same parse.  Re-parsing the projected text
        # works because the preamble is free text ahead of the diff start, which is the
        # same skip a DEP-3 header gets.
        marked = {entry['path'] for entry in self.files}
        expected = [section.path for section in content_mod._parse_sections(GATOS_SHAPED)
                    if section.path not in marked]
        self.assertEqual(['Makefile.am.ori', 'src/foo.c'], expected)
        self.assertEqual(expected,
                         [section.path for section in content_mod._parse_sections(self.result.text)])

    def test_projection_starts_at_the_first_file_header(self):
        # The documented contract: callers pass the diff body (``triage.diff_body`` has
        # already stripped the DEP-3 header). A body that still carries one loses it --
        # free text belongs to no file, so there is no segment to quote it in.
        body = 'Description: regenerate the build system\nOrigin: upstream\n\n%s' % GATOS_SHAPED
        result = generated.project_residue_first(body, _evidence_files(body))
        self.assertTrue(result.projected)
        self.assertNotIn('Description: regenerate', result.text)
        self.assertIn(RESIDUE_SEGMENTS[1], result.text)

    def test_an_all_generated_body_projects_to_preamble_and_notes(self):
        body = _file_diff('configure', ['+ac_new_one'])
        result = generated.project_residue_first(body, _evidence_files(body))
        self.assertTrue(result.projected)
        self.assertEqual(2, len(result.text.splitlines()))
        self.assertEqual(1, result.omitted_files)


class ProjectionIdentityTestCase(testtools.TestCase):
    """Nothing to project: the body is returned untouched, so callers never special-case."""

    def test_no_marked_files_is_the_identity(self):
        body = _edit('src/foo.c') + _edit('doc/foo.1')
        result = generated.project_residue_first(body, [])
        self.assertEqual(body, result.text)
        self.assertFalse(result.projected)
        self.assertEqual(0, result.omitted_files)
        self.assertEqual(0, result.omitted_changed)

    def test_a_marked_path_absent_from_the_body_is_the_identity(self):
        # A mark whose paths do not appear here (a mismatched pairing) omits nothing and
        # must not silently rewrite an untouched body into a preamble with no notes.
        body = _edit('src/bar.c')
        result = generated.project_residue_first(body, _evidence_files(GATOS_SHAPED))
        self.assertEqual(body, result.text)
        self.assertFalse(result.projected)
        self.assertEqual(0, result.omitted_changed)

    def test_an_empty_body_is_the_identity(self):
        result = generated.project_residue_first('', _evidence_files(GATOS_SHAPED))
        self.assertEqual('', result.text)
        self.assertFalse(result.projected)


# ---------------------------------------------------------------------------
# Construct-vs-residue tally: render-time display over one body
# ---------------------------------------------------------------------------

# gatos's construct story in miniature.  The real patch carries 128 ``shell-out``
# observations, every one of them backtick command substitution in the generated
# ``configure``/``ltmain.sh`` shell; here one lands in a marked ``ltmain.sh``, one in the
# hand-written residue, and two more sit in files the SCAN never looks at -- a doc file
# and (name-marked but build-typed) ``configure`` -- so the type gate is exercised in
# both directions.
CONSTRUCTS_SHAPED = ''.join([
    # Marked by name, typed ``code`` (``.sh``): its added backtick line is a real hit.
    _file_diff('ltmain.sh', [
        ' # ltmain.sh (GNU libtool) 1.5.22',
        '+  func_dir=`dirname "$file"`']),
    # The hand-written residue: unmarked, typed code, and shelling out.
    _file_diff('src/foo.c', ['+    system(cmd);']),
    # Prose naming the same construct: typed doc, so the scan never sees it.
    _file_diff('doc/manual.md', ['+    system(cmd);']),
    # Marked by name but typed ``build``, so its shell is not scanned either.
    _file_diff('configure', [
        ' # Generated by GNU Autoconf 2.59.',
        '+  ac_dir=`dirname "$ac_file"`']),
])

MARKED_PATHS = frozenset(['ltmain.sh', 'configure'])


class ConstructTallyTestCase(testtools.TestCase):
    """The split a reviewer reads: how many construct hits are in generated-claiming
    files, and how many are in the residue they actually have to review."""

    def setUp(self):
        super().setUp()
        self.tally = generated.construct_tally(CONSTRUCTS_SHAPED, MARKED_PATHS)

    def test_the_marked_files_hit_is_tallied_as_marked(self):
        self.assertEqual(1, self.tally.in_marked)

    def test_the_residue_hit_is_tallied_as_residue(self):
        self.assertEqual(1, self.tally.in_residue)

    def test_the_residue_hit_names_its_file_and_pattern(self):
        self.assertEqual((('src/foo.c', 'shell-out'),), self.tally.residue_hits)

    def test_the_parts_sum_to_the_total(self):
        self.assertEqual(2, self.tally.total)
        self.assertEqual(self.tally.total, self.tally.in_marked + self.tally.in_residue)

    def test_a_construct_in_prose_counts_nowhere(self):
        # The type gate the recorded scan applies: a doc file's ``system(`` never flagged
        # when the observation was recorded, so it must not appear in the recount either.
        self.assertNotIn('doc/manual.md', [path for path, _detail in self.tally.residue_hits])
        doc_only = generated.construct_tally(
            _file_diff('doc/manual.md', ['+    system(cmd);']), frozenset())
        self.assertEqual(0, doc_only.total)

    def test_a_construct_in_a_build_file_counts_nowhere(self):
        # ``configure`` is name-marked AND full of shell, but it is typed ``build``, so
        # the scan never reaches it -- exactly as when the observations were recorded.
        configure_only = generated.construct_tally(
            _file_diff('configure', ['+  ac_dir=`dirname "$ac_file"`']), frozenset(['configure']))
        self.assertEqual(0, configure_only.total)

    def test_the_tally_matches_the_recorded_scan_total(self):
        # The recount of one body sees exactly the flags the deterministic scan of that
        # same body raises -- the split is the only thing added.
        from divergulent.classify import rules as rules_mod
        self.assertEqual(len(rules_mod.scan_dangerous_constructs(CONSTRUCTS_SHAPED)),
                         self.tally.total)

    def test_an_empty_marked_set_puts_everything_in_the_residue(self):
        # The honest default: with nothing claiming generation, no hit is excused.
        tally = generated.construct_tally(CONSTRUCTS_SHAPED, frozenset())
        self.assertEqual(2, tally.total)
        self.assertEqual(0, tally.in_marked)
        self.assertEqual(2, tally.in_residue)
        self.assertEqual((('ltmain.sh', 'shell-out'), ('src/foo.c', 'shell-out')),
                         tally.residue_hits)

    def test_a_body_with_no_constructs_tallies_zero(self):
        tally = generated.construct_tally(GATOS_SHAPED, frozenset(['configure', 'src/Makefile.in']))
        self.assertEqual(0, tally.total)
        self.assertEqual(0, tally.in_marked)
        self.assertEqual(0, tally.in_residue)
        self.assertEqual((), tally.residue_hits)

    def test_an_empty_body_tallies_zero(self):
        self.assertEqual(0, generated.construct_tally('', frozenset()).total)

    def test_repeated_hits_in_one_file_are_one_residue_entry_but_counted_each(self):
        body = _file_diff('src/foo.c', ['+    system(a);', '+    system(b);'])
        tally = generated.construct_tally(body, frozenset())
        self.assertEqual(2, tally.in_residue)                       # both counted
        self.assertEqual((('src/foo.c', 'shell-out'),), tally.residue_hits)  # one fact

    def test_the_ledger_is_never_touched(self):
        # Pure display: the helper takes a body and a path set, and returns counts. No
        # connection is accepted, so no observation can be read or rewritten.
        import inspect
        signature = inspect.signature(generated.construct_tally)
        self.assertEqual(['body', 'marked_paths'], list(signature.parameters))


class ConstructTallySingleSourcingTestCase(testtools.TestCase):
    """The tally reuses rules.py's pattern tables -- never a second copy of them."""

    def test_a_pattern_added_to_the_rules_table_flows_into_the_tally(self):
        # Identity, not equality: the tally reaches the LIVE table object through
        # ``rules``, so a pattern added there is tallied with no change here.
        from unittest import mock
        from divergulent.classify import rules as rules_mod
        body = _file_diff('src/foo.c', ['+    launch_the_missiles();'])
        self.assertEqual(0, generated.construct_tally(body, frozenset()).total)
        extended = rules_mod._DANGEROUS_PATTERNS + (
            ('novel-construct', re.compile(r'\blaunch_the_missiles\s*\(')),)
        with mock.patch.object(rules_mod, '_DANGEROUS_PATTERNS', extended):
            tally = generated.construct_tally(body, frozenset())
        self.assertEqual(1, tally.total)
        self.assertEqual((('src/foo.c', 'novel-construct'),), tally.residue_hits)

    def test_the_shell_only_table_flows_in_too(self):
        # Both tables, so shell scoping (backticks flag in shell, not in JS) is the
        # scan's decision and not re-made here.
        from unittest import mock
        from divergulent.classify import rules as rules_mod
        body = _file_diff('src/tool.sh', ['+  x=`echo hi`'])
        with mock.patch.object(rules_mod, '_SHELL_DANGEROUS_PATTERNS', ()):
            self.assertEqual(0, generated.construct_tally(body, frozenset()).total)
        self.assertEqual(1, generated.construct_tally(body, frozenset()).total)

    def test_the_tally_defines_no_patterns_of_its_own(self):
        import inspect
        source = inspect.getsource(generated)
        self.assertIn('scan_dangerous_constructs_by_file', source)
        self.assertNotIn('_DANGEROUS_PATTERNS', source)

    def test_the_per_file_scan_is_the_same_scan(self):
        # ``scan_dangerous_constructs`` is defined as the per-file scan with the
        # attribution dropped, so the recorded flags and the recount cannot diverge.
        from divergulent.classify import rules as rules_mod
        self.assertEqual(
            rules_mod.scan_dangerous_constructs(CONSTRUCTS_SHAPED),
            [flag for _path, flag in rules_mod.scan_dangerous_constructs_by_file(
                CONSTRUCTS_SHAPED)])
...
# ---------------------------------------------------------------------------
# The oversized unlock: two live observations composed in one place
# ---------------------------------------------------------------------------
...
FP_BIG_RESIDUE = '3' * 64
FP_NO_MARK = '4' * 64
FP_LARGE_MARKED = '5' * 64
FP_BAD_EVIDENCE = '6' * 64


class ResidueUnlockedFingerprintsTestCase(testtools.TestCase):
    """The composition the triage driver and the risk gate both subtract."""
    ...
    def setUp(self):
        ...
        # gatos itself: a 47k-line patch whose hand-written residue is 603 lines.
        self._oversized(FP_UNLOCKED, total=47000)
        self._mark(FP_UNLOCKED, residue=603, total=47000)

        # The threshold edge: a residue EXACTLY at the oversized cut is unlocked (``<=``).
        self._oversized(FP_EDGE, total=20000)
        self._mark(FP_EDGE, residue=reviewability_mod.REVIEWABILITY_OVERSIZED_LINES, total=20000)

        # gatos's sibling: residue 5,410, over the cut. Honestly still unreviewable.
        self._oversized(FP_BIG_RESIDUE, total=30000)
        self._mark(FP_BIG_RESIDUE, residue=5410, total=30000)

        # Oversized and nothing claimed generation: stays locked, as before this phase.
        self._oversized(FP_NO_MARK, total=9000)

        # Marked, but never locked in the first place -- the unlock has nothing to open.
        self._reviewability(FP_LARGE_MARKED, 'large', total=900)
        self._mark(FP_LARGE_MARKED, residue=10, total=900)

        # Oversized with a mark whose evidence is unreadable: ``generated_marks`` drops the
        # row, so the fingerprint reads as unmarked and stays locked.
        self._oversized(FP_BAD_EVIDENCE, total=12000)
        ledger_mod.append_observation(
            self.conn, fingerprint=FP_BAD_EVIDENCE, kind=generated.GENERATED_KIND,
            detail='autotools/99', evidence='not json at all {',
            observed_by=generated.GENERATED_OBSERVED_BY,
            rule_version=generated.GENERATED_RULES_VERSION, observed_at=WHEN)

    def _reviewability(self, fingerprint, level, total):
        ledger_mod.append_observation(
            self.conn, fingerprint=fingerprint, kind=reviewability_mod.REVIEWABILITY_KIND,
            detail=level, evidence=json.dumps({'changed_lines': total}, sort_keys=True),
            observed_by=reviewability_mod.REVIEWABILITY_OBSERVED_BY,
            rule_version=reviewability_mod.REVIEWABILITY_VERSION, observed_at=WHEN)

    def _oversized(self, fingerprint, total):
        self._reviewability(fingerprint, 'oversized', total)

    def _mark(self, fingerprint, residue, total):
        evidence = json.dumps(
            {'files': [{'path': 'configure', 'family': 'autotools', 'signals': ['name'],
                        'generator': None, 'version': None,
                        'added': total - residue, 'removed': 0}],
             'generated_changed': total - residue,
             'residue_changed': residue,
             'total_changed': total},
            sort_keys=True)
        ledger_mod.append_observation(
            self.conn, fingerprint=fingerprint, kind=generated.GENERATED_KIND,
            detail='autotools/99', evidence=evidence,
            observed_by=generated.GENERATED_OBSERVED_BY,
            rule_version=generated.GENERATED_RULES_VERSION, observed_at=WHEN)

    def test_the_unlocked_set_is_exactly_the_small_residue_oversized(self):
        self.assertEqual([FP_UNLOCKED, FP_EDGE],
                         sorted(generated.residue_unlocked_fingerprints(self.conn)))

    def test_a_small_residue_oversized_patch_is_unlocked(self):
        self.assertIn(FP_UNLOCKED, generated.residue_unlocked_fingerprints(self.conn))

    def test_the_threshold_edge_is_inclusive(self):
        # ``residue_changed <= REVIEWABILITY_OVERSIZED_LINES`` is the contract: the same
        # cut reviewability applies to the total, applied to the residue.
        self.assertIn(FP_EDGE, generated.residue_unlocked_fingerprints(self.conn))

    def test_a_big_residue_stays_locked(self):
        self.assertNotIn(FP_BIG_RESIDUE, generated.residue_unlocked_fingerprints(self.conn))

    def test_an_oversized_patch_with_no_mark_stays_locked(self):
        self.assertNotIn(FP_NO_MARK, generated.residue_unlocked_fingerprints(self.conn))

    def test_malformed_evidence_reads_as_no_mark(self):
        self.assertNotIn(FP_BAD_EVIDENCE, generated.residue_unlocked_fingerprints(self.conn))

    def test_a_marked_but_not_oversized_patch_is_not_in_the_set(self):
        # It was never locked; the unlock is a subtraction from the skip set, not a grant.
        self.assertNotIn(FP_LARGE_MARKED, generated.residue_unlocked_fingerprints(self.conn))

    def test_a_superseded_mark_no_longer_unlocks(self):
        ledger_mod.supersede_observations_for_fingerprint(
            self.conn, fingerprint=FP_UNLOCKED, kind=generated.GENERATED_KIND,
            superseded_at='2026-07-27T00:00:00+00:00')
        self.assertNotIn(FP_UNLOCKED, generated.residue_unlocked_fingerprints(self.conn))

    def test_empty_ledger_unlocks_nothing(self):
        conn = ledger_mod.create_ledger(os.path.join(self.tmp.name, 'empty.sqlite'))
        self.addCleanup(conn.close)
        self.assertEqual(set(), generated.residue_unlocked_fingerprints(conn))
...
class TranslationCandidateTestCase(testtools.TestCase):
    ...
    def test_scan_never_marks_a_translation_candidate(self):
        # The candidate is measurement only: the shipped scanner must not mark on it.
        self.assertEqual((), generated.scan(QT_TS_NEW_FILE).files)
        self.assertEqual((), generated.scan(PO_UPDATE_HUNK).files)
