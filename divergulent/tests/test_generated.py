...
class NameSignalTestCase(testtools.TestCase):
    ...
    def test_name_only_match_has_the_name_signal_alone(self):
        entry = _marked(_edit('Makefile.in'))['Makefile.in']
...
class BackupSuffixTestCase(testtools.TestCase):
    ...
    def test_backup_of_a_generated_file_is_marked(self):
        ...
        self.assertIn('Makefile.in.ori', _marked(_edit('Makefile.in.ori')))
...
class BannerSignalTestCase(testtools.TestCase):
    ...
    def test_libtool_product_name_banner_without_version(self):
        entry = self._banner('# This is GNU Libtool, a generic library support script.')
        self.assertEqual('libtool', entry.generator)
        self.assertIsNone(entry.version)
    ...
    def test_first_banner_per_file_wins(self):
        # One generator per file, never an accumulation of every banner present.
...
class TimestampVersionTestCase(testtools.TestCase):
    ...
    def test_timestamp_on_another_generated_name_does_nothing(self):
        # The datestamp is only meaningful for the two names that carry one.
        text = _file_diff('Makefile.in', [" timestamp='2005-07-08'", '+all:'])
...
GATOS_SHAPED = ''.join([
    ...
    _file_diff('configure', [
        ...
        '+ac_new_three',
        '+ac_new_four']),
    # Regenerated Makefile.in, no banner in the touched region (1 removed, 1 added).
    _file_diff('src/Makefile.in', ['-CFLAGS = -O1', '+CFLAGS = -O2']),
...
class GatosShapedPatchTestCase(testtools.TestCase):
    ...
    def test_makefile_in_is_name_only(self):
        self.assertEqual(('name',), self.by_path['src/Makefile.in'].signals)
