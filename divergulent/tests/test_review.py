...
# gatos in miniature: a regenerated ``configure`` carrying its autoconf banner (so the
# scan captures a generator AND a version) ahead of the one hand-written hunk the
# reviewer actually needs to read.  Generator output FIRST, as it is in the real patches.
# Scans to one marked file, ``autotools/60``, 3 generated and 2 residue changed lines.
MARKED_PATCH = (
    ...
    '+ac_cv_generated_a=yes\n'
    '+ac_cv_generated_b=yes\n'
    '+ac_cv_generated_c=yes\n'
    '--- a/src/reader.c\n'
    '+++ b/src/reader.c\n'
    '@@ -3,3 +3,3 @@\n'
    ' int read_line(FILE *fp) {\n'
...
def _build_corpus(corpus_dir, patch=PATCH):
...
class ReviewFixture:
    ...
    def _setup(self, *, draft_category='bugfix', patch=PATCH):
    ...
    def _mark(self, conn, fingerprint_hex, patch_text):
        """Record the live ``generated-content`` observation the review view reads.

        Built from a real ``generated.scan`` of the fixture body through ``detail_for``/
        ``evidence_for`` -- never hand-crafted evidence JSON -- so the context is exercised
        against exactly the row the deterministic record pass writes.
        """
        scan = generated.scan(patch_text)
        ledger_mod.append_observation(
            conn, fingerprint=fingerprint_hex, kind=generated.GENERATED_KIND,
...
class GeneratedMarkContextTestCase(ReviewFixture, testtools.TestCase):
    """The context carries the fingerprint's live generated-content mark, or nothing."""

    def _context_for(self, conn, corpus_dir, index_path, fp_hex):
        return review.build_review_context(
            conn, corpus_dir, index_path, fingerprint=fp_hex, fetch=_recording_fetch()[0])

    def test_marked_fingerprint_carries_detail_paths_and_residue(self):
        conn, corpus_dir, index_path, fp_hex = self._setup(patch=MARKED_PATCH)
        self._mark(conn, fp_hex, MARKED_PATCH)

        context = self._context_for(conn, corpus_dir, index_path, fp_hex)

        scan = generated.scan(MARKED_PATCH)
        self.assertEqual(generated.detail_for(scan), context.generated_detail)
        self.assertEqual('autotools/60', context.generated_detail)
        # Only the file that CLAIMS generation; the hand-written residue is not in the set.
        self.assertEqual(frozenset(['configure']), context.generated_paths)
        self.assertEqual((scan.residue_changed, scan.generated_changed), context.generated_residue)
        self.assertEqual((2, 3), context.generated_residue)

    def test_unmarked_fingerprint_carries_no_mark(self):
        conn, corpus_dir, index_path, fp_hex = self._setup()
        context = self._context_for(conn, corpus_dir, index_path, fp_hex)
        self.assertIsNone(context.generated_detail)
        self.assertEqual(frozenset(), context.generated_paths)
        self.assertIsNone(context.generated_residue)

    def test_another_fingerprints_mark_is_not_borrowed(self):
        # The marks are read whole (one ledger pass, as every other axis consumer does);
        # the context must pick out its own fingerprint and no one else's.
        conn, corpus_dir, index_path, fp_hex = self._setup()
        self._mark(conn, 'b' * 64, MARKED_PATCH)
        context = self._context_for(conn, corpus_dir, index_path, fp_hex)
        self.assertIsNone(context.generated_detail)
        self.assertEqual(frozenset(), context.generated_paths)

    def test_a_superseded_mark_is_not_read(self):
        # Retraction is a supersede: an un-marked fingerprint stops being tagged.
        conn, corpus_dir, index_path, fp_hex = self._setup(patch=MARKED_PATCH)
        self._mark(conn, fp_hex, MARKED_PATCH)
        ledger_mod.supersede_observations_for_fingerprint(
            conn, fingerprint=fp_hex, kind=generated.GENERATED_KIND, superseded_at=WHEN)
        context = self._context_for(conn, corpus_dir, index_path, fp_hex)
        self.assertIsNone(context.generated_detail)
...
class FormatFileListTestCase(testtools.TestCase):
    ...
    def test_marked_rows_are_tagged_and_only_those(self):
        lines = review._format_file_list(
            [review.FileStat('src/hand-edit.c', 1, 0),
             review.FileStat('Makefile.in', 20, 20),
             review.FileStat('configure', 100, 50)],
            frozenset(['configure', 'Makefile.in']))
        self.assertTrue(lines[1].endswith('configure [gen]'), lines[1])
        self.assertTrue(lines[2].endswith('Makefile.in [gen]'), lines[2])
        self.assertTrue(lines[3].endswith('src/hand-edit.c'), lines[3])
        self.assertNotIn('[gen]', lines[3])   # the residue is not tagged
        self.assertNotIn('[gen]', lines[0])   # nor is the totals line

    def test_a_marked_path_this_diff_does_not_touch_tags_nothing(self):
        # The mark is read whole; only the rows this diff actually has are tagged.
        lines = review._format_file_list(
            [review.FileStat('src/hand-edit.c', 1, 0)], frozenset(['configure']))
        self.assertNotIn('[gen]', '\n'.join(lines))

    def test_unmarked_list_is_byte_identical_to_the_untagged_call(self):
        stats = [review.FileStat('configure', 100, 50), review.FileStat('src/hand-edit.c', 1, 0)]
        self.assertEqual(review._format_file_list(stats),
                         review._format_file_list(stats, frozenset()))
        self.assertNotIn('[gen]', '\n'.join(review._format_file_list(stats)))


class FormatGeneratedSummaryTestCase(testtools.TestCase):
    """The one mark-summary line under the totals: present when marked, absent when not."""

    STATS = [review.FileStat('configure', 100, 50), review.FileStat('src/reader.c', 1, 1)]

    def test_unmarked_context_adds_no_line(self):
        self.assertEqual(
            [], review._format_generated_summary(_context(packages=['reader']), self.STATS))

    def test_summary_names_the_counts_the_detail_and_the_residue(self):
        context = _context(
            packages=['reader'], generated_detail='autotools/99',
            generated_paths=frozenset(['configure']), generated_residue=(603, 46000))
        self.assertEqual(
            ['generated-claiming: 1 of 2 files marked (autotools/99); residue 603 lines changed'],
            review._format_generated_summary(context, self.STATS))

    def test_only_the_files_this_diff_touches_are_counted(self):
        # The mark names a file the representative diff does not touch: the summary
        # describes the list it heads, so that file is not counted into it.
        context = _context(
            packages=['reader'], generated_detail='autotools/99',
...
def _rendered_view(context, choice='defer'):
    ...
    ``_interactive_ask`` pages through ``_page``, which falls back to a plain ``print``
    when stdout is not a TTY (it never is under the test runner), so capturing stdout
    captures the view.  stdin is scripted so the verdict prompt returns at once.
    """
    ...
    try:
        with contextlib.redirect_stdout(buffer):
            review._interactive_ask(context)
    finally:
        sys.stdin = original_stdin
    return buffer.getvalue()


class InteractiveViewMarkTestCase(ReviewFixture, testtools.TestCase):
    """The view a reviewer actually sees: tagged rows and one summary line, or neither."""

    def _view(self, patch, *, mark=False):
        conn, corpus_dir, index_path, fp_hex = self._setup(patch=patch)
        if mark:
            self._mark(conn, fp_hex, patch)
        context = review.build_review_context(
            conn, corpus_dir, index_path, fingerprint=fp_hex, fetch=_recording_fetch()[0])
        return context, _rendered_view(context)

    def test_marked_view_leads_with_the_summary_then_tags_the_marked_row(self):
        _context_unused, view = self._view(MARKED_PATCH, mark=True)
        lines = view.splitlines()
        totals = lines.index('files changed: 2 (+4 / -1)')
        self.assertEqual(
            'generated-claiming: 1 of 2 files marked (autotools/60); residue 2 lines changed',
            lines[totals + 1])
        self.assertTrue(lines[totals + 2].endswith('configure [gen]'), lines[totals + 2])
        self.assertTrue(lines[totals + 3].endswith('src/reader.c'), lines[totals + 3])
        self.assertNotIn('[gen]', lines[totals + 3])
        # Tagged, never hidden: the marked file's diff block is still rendered in full.
        self.assertIn('### configure', view)
        self.assertIn('ac_cv_generated_a=yes', view)

    def test_unmarked_view_renders_exactly_as_it_did_before_the_mark(self):
        context, view = self._view(PATCH)
        self.assertNotIn('[gen]', view)
        self.assertNotIn('generated-claiming', view)
        # The file-list block is byte-for-byte what the untagged formatter -- the code
    ...
    def test_an_unmarked_patch_in_a_ledger_with_marks_is_untouched(self):
        ...
        conn, corpus_dir, index_path, fp_hex = self._setup()
        self._mark(conn, 'c' * 64, MARKED_PATCH)
        context = review.build_review_context(
            conn, corpus_dir, index_path, fingerprint=fp_hex, fetch=_recording_fetch()[0])
        view = _rendered_view(context)
        self.assertNotIn('[gen]', view)
        self.assertNotIn('generated-claiming', view)
