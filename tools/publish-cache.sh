...
# upload with `gh release upload --clobber`, which overwrites the previous day's
...
# it against GitHub's published checksums if absent (the same pattern release.yml
# uses for gitsign).
ensure_gh() {
