...
2. For a brand-new project that does not yet exist on PyPI, use **Your
   account** > **Publishing** > **Add a new pending publisher**. For an
   existing project, navigate to it and use **Settings** > **Publishing**.
3. Fill in the form:
   - **PyPI Project Name**: `divergulent`
   - **Owner**: `shakenfist`
   - **Repository name**: `divergulent`
...
1. Go to the repository on GitHub: `shakenfist/divergulent`
...
The package version is derived from the git tag by `setuptools_scm`, so the
tag is the single source of truth for the released version.
